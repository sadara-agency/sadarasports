...
/**
 * Version 2 — home-only pillar variant with magnetic hover + electric underline.
 *
 * Mirrors PillarNav's `size="home"` markup (pillars + secondary + children column)
 * but adds: (1) each pillar link nudges a few px toward the cursor (spring-smoothed)
 * and (2) an electric-blue underline that wipes in from the start edge on hover.
 *
 * Kept SEPARATE from PillarNav so the shared MegaNav overlay is untouched.
 * Reduced-motion → no magnetic nudge, underline appears instantly (no wipe).
 */
export function MagneticPillars_v2({ locale }: { locale: Locale }) {
...
function MagneticPillar({
  ...
  const onMove = (e: React.MouseEvent) => {
    ...
    // Pull a few px toward the cursor (scaled down).
    mx.set(relX * 0.06);
    my.set(relY * 0.12);
  };
  const reset = () => {
  ...
  return (
    ...
    >
      <motion.div style={{ x, y }}>
        {children}
        {/* Electric underline — wipes from the start edge on hover (direction-aware via inset-inline-start). */}
        <span
          aria-hidden="true"
